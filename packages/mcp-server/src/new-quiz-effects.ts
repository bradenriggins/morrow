...
export async function planNewQuizReport(runtime: EffectsRuntime, value: z.infer<typeof reportInput>, callerSignal?: AbortSignal): Promise<CallToolResult> {
  try {
    ...
    return { ...planned, content: [{ type: "text", text: `Review one ${input.format} ${input.report_type} report request for ${title}. Canvas must return an exact Assignment-bound Progress receipt. Open the report from this quiz in Canvas after it completes. No Canvas request has been made or scheduled.` }], structuredContent: isJsonObject(planned.structuredContent) ? { ...planned.structuredContent, new_quiz_effect_plan: { schema: "morrow.new-quiz-effect.plan.v1", action: "report", target: { course_id: input.course_id, quiz_id: input.quiz_id }, payload, operation_count: 1 } } : {} } as CallToolResult;
  } catch (error) { return problem(error); }
}
...
export function registerNewQuizEffectTools(server: McpServer, runtime: GatewayRuntime): void {
  ...
  server.registerTool("morrow_plan_new_quiz_report", { title: "Review a New Quiz report", description: "Read the exact Canvas course and New Quiz, validate the report type and format, and return one report request for review. The response must be an exact Assignment-bound Progress record. Open the report from the same quiz in Canvas after it completes.", inputSchema: reportInput, annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true } }, (input, context) => planNewQuizReport(runtime, input, context.mcpReq.signal));
}

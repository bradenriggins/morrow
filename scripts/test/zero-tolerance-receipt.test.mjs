...
function writeFixture() {
  ...
  writeFileSync(resolve(evidence, "Morrow-1.0.2-win-x64.exe"), installer);
  ...
  writeFileSync(resolve(evidence, "smoke.harness.json"), JSON.stringify({
    ...
    installer: { fileName: "Morrow-1.0.2-win-x64.exe" },
  ...
  writeFileSync(resolve(evidence, "upgrade.json"), JSON.stringify({
    ...
    registration: { displayName: "Morrow 1.0.2", displayVersion: "1.0.2", publisher: "Braden Riggins" },
    uninstall: {
      completed: true,
...
test("only the native-Windows ACL skip is accepted, and only with bound evidence", (t) => {
  ...
  writeFileSync(resolve(evidence, "Morrow-1.0.2-win-x64.exe"), "different installer");
  assert.throws(() => validateTestOutput({
    id: "workspace-test",
    output: SKIP_LOG,
    repositoryRoot: root,

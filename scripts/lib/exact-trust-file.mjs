...
function exactMaximum(value, label) {
...
export function readExactTrustFile(pathValue, { label, maxBytes }) {
  const maximum = exactMaximum(maxBytes, label);
  ...
  if (!canonicalParentIdentity.isDirectory()) throw new Error(`${label} parent is not a directory`);
  ...
  try {
    ...
    if (length !== Number(opened.size)) throw new Error(`${label} changed while it was read`);
...
export function readExactTrustJson(path, options) {
  let text;
  try {

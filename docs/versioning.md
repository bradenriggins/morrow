...
4. Record the release in the product's own changelog (`desktop/` keeps its release notes; `morrow-for-muse/CHANGELOG.md` covers Muse).
...
- Neither changed (docs, root README, workflow edits): both suites skip, and the required `check` still reports success.

...
and wrote `docs/research/CANVAS-NEW-QUIZZES-ITEM-BANKS-CONTRACT-2026-09-06.md` (304 lines): the three surfaces (`/api/quiz/v1/`, `/api/banks/` inside the course-bound Item Banks LTI frame, and the Canvas REST assignment surface), request and response shapes per interaction type, the fan-out precondition for editing an existing bank, freezing an operation, item payload validation, failure semantics (the classifier: only 4xx except 408 and 429 is "definitely not applied"; everything else is indeterminate and never retryable), role permissions, and what the harvest does not establish. No credentials, hostnames, or student data are in the file.

Evaluation of Morrow against it (17 findings, 2 critical): the in-frame executor asserted `outcomeUnknown: false` for every non-2xx status, the exact inverse of the harvested classifier, so a 5xx on a shared-bank write invited a retry that double-applies (also present at `canvas-content.js:874` for New Quizzes); the service worker injected the Item Bank operation, including the full payload, into every frame on the tab. High: no `GET /api/banks/{bank_id}/items/{item_id}` so `update_item` had no exact comparator; `create_item` readback targeted a list the created item is not yet in; the `banks.build` storage contract was an assumption; `canvas_update_quiz_item` had no interaction-id preservation rule despite a recorded production incident (10 real plus 18 ghost choices); form-encoded writes where the harvest uses JSON; no fan-out record even though `list_shares` was in the catalog and called nowhere.

All 20 items are done:

- nq-01 executor classifies by the harvested rule; nq-02 execution restricted to Item Banks frames (`connector/extension/src/item-bank-frames.js`, host pattern `^[^.]+\.quiz-(?:lti|api)(?:-[^.]+)*\.instructure\.com$`) and the probe no longer carries the payload; nq-03 `canvas_item_bank_get_item` added and `list_shares` paginated; nq-04 item writes read the exact item.
- nq-05 to nq-08: the affected-course fan-out record (from `list_shares`), its MCP reader, the in-frame entry guard with fresh read before and after one dispatch, and one exact Edit category with its admission rule.
- nq-09 to nq-11: the MCP planner, audit-planner repair plans for New Quiz items and bank entries, inventory integration.
- nq-12 to nq-14: New Quiz interaction-id preservation on update (`scripts/test/canvas-new-quiz-write-contract.test.mjs`), JSON encoding on every `/quiz/v1` write, quiz-settings read-merge-preserve.
- nq-15 to nq-18: item lifecycle (create, read, update, delete), payload validators per interaction type, the seven-case runtime test matrix (stale source, incomplete fan-out, changed fan-out, one dispatch, readback mismatch, uncertain refusal, replay), the tool surface and approval UX.
- nq-19 documentation reconciliation with a claim test; nq-20 `docs/implementation/ITEM-BANK-LIVE-PROOF-PLAN.md`, the live-proof checklist for the day an authorized course is available.

Still true: no live New Quiz or Item Bank write has been made from Morrow. The `banks.build` credential handling is fixture-proven only. Stimulus repair is not claimed; the harvest does not establish a complete preservation contract for every Stimulus field, as the 6 September handoff warned.

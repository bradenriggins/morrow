...
```bash
MORROW_LEGACY_ROOT=/absolute/path/to/morrow-legacy pnpm bridge:legacy:remove
```
...
Meridian remains the preferred source for exact-name collisions in the example. Morrow legacy retains its unique tool names and receives deterministic aliases when both donors advertise the same public name.

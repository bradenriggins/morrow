...
### 431: Bridge rollback is coupled to assistant repair

- Verified defect: the first live **Restore previous Bridge** action entered full setup repair. A permitted unrelated edit in the ChatGPT configuration then returned `existing_morrow_configuration` and left the staged Bridge update in place.
- Root cause: the renderer used the broad repair IPC action for a narrow Bridge transaction. That action also repairs the installer record and assistant configuration.
- Repair: Desktop now exposes one input-free `installer:restore-bridge` channel. It stops the runtime under the existing desktop mutation guard, verifies the pending Bridge state, rolls back and resumes the exact fenced worker, refreshes runtime state, and never touches assistant configuration.
...
### 437: a truncated list is published as a complete result

- Verified defect: `canvas_list_pages_courses` on live BT2 course `89585` returned 250 rows with the source result reporting `pageCount: 25` and `truncated: true`. The Morrow envelope reported `completeness: "complete"` with empty `limitations` and empty `attention`. The course page created at `2026-09-15T03:23:50Z` was absent from that list, and a direct read of `morrow-live-page-1789442624` returned it with HTTP 200.
- Root cause: the public read envelope was built from fixed values only. `completeness` therefore fell to its default, which is `complete` for any read that did not raise an error. The source result already carried its own bound, and Morrow's internal readers already refuse it: `canvasReadResult` rejects `truncated !== false`, and the course inventory reads `truncated`, `pageCount`, and `morrow_unread_pages`. Only the public capability read discarded that evidence.
- Repair: the read envelope now derives its completeness from the source result. A Canvas browser read that reports `truncated: true` is published as `limited` with one fixed limitation that names the page bound and, when the source reports it, the number of unread provider pages.
...
### 484: a change with no readable result could never be settled
...
### 497: a review looked past the record Canvas returned

- Product decision: a review that has read the object it names can name it.
- Condition: Canvas answers some routes with the record inside an envelope it names, as `{"polls": [ ... ]}` from `/v1/polls/{id}`. A review read the answer, looked for an id at the top level, found none, and reported "Canvas does not have the item this change names. It may have been renamed, moved, or removed". The poll was there and Canvas had just returned it. The approval control is withheld whenever a target cannot be named, so every change to a poll, a poll choice or a poll session was unapprovable.
- Repair: the review opens the envelope the answer carries and takes the record that matches the object the change names. A record for another object is still reported as gone, so an answer about something else never names this change's target. A review also names a poll by its question, a blackout date by its event title, and a poll choice by its text, because that is what Canvas calls them.
- Status: `IMPLEMENTED`; covered by `packages/mcp-server/test/approval-context.test.ts`, which also holds the case where Canvas answers about another poll and the review still reports it gone.

### 498: Morrow could not recognise the person it had named itself

- Product decision: what Morrow names a person by is what Morrow recognises that person by.
- Condition: Morrow names a person in a course by the learner token it issues, never by a raw Canvas id, and the record it reads back carries that token in place of the id. A review resolved a submission by looking for an id, a url or the route's own field on the record it read, and the record carried none of them. The submission Morrow had just read successfully was reported as "Canvas does not have the item this change names", and the approval control is withheld whenever a target cannot be named, so every change to a submission could never be approved: marking one read, grading one, reading its rubric assessment or its annotations.

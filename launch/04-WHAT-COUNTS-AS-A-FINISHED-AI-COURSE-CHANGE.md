...
Each change belongs to a current account, explicit course, provider, and named record. Those conditions must travel with the request.

This prevents a valid instruction from acting on the wrong place after a session, course, tab, or target changes. If the saved operation no longer matches the current context, it stops for a fresh review.

Canvas page records include URL, title, body, publication, front-page, editing-role, update-time, and revision fields. They let a team prove more than a changed sentence. [Canvas Pages API](https://canvas.instructure.com/doc/api/pages.html)

## Make the change readable

The review should name the current value, proposed value, record, and preserved conditions. “Fix the biology page” asks for interpretation. “Replace this sentence while preserving these fields” is reviewable.
...
In the Canvas test, fresh readback verified one intended lesson phrase change. The page title, URL, publication state, front-page state, and editing roles stayed unchanged.

The related New Quiz correction also reached verified readback. The exact comparison showed that only question one’s saved scoring value and update time changed; the other question fields stayed unchanged. Two module-link writes and three publication operations each reached verified readback.
...
Canvas has selected live sandbox evidence through a signed-in Chrome connection. Moodle has six current tools: five reads and one course-summary write. Blackboard also has six: five reads, including direct Ultra child discovery, and one supported document title/body update. Moodle and Blackboard use separate private API connections.

Those Moodle and Blackboard scopes have official API and test validation, not live-tenant evidence. Provider-specific scope prevents one provider's evidence from becoming another provider's claim.

...
This file is an index. The accepted decisions live in the ADRs. Do not flatten Meridian and Morrow legacy into one implementation.
...
- Meridian runs as an internal stdio MCP upstream.
- The existing Morrow capability catalog is exported from `morrow-legacy` through a deterministic donor-side inventory script.

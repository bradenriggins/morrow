...
Before it sends a question a caller supplied to a bank, the connector checks that payload: every image names its alternative text, every image, audio, and video source is an `https://` URL or a Canvas file path, and the choice, matching, numeric, and rich fill in the blank shapes carry the ids, answers, and answer keys those shapes need. Every other interaction type is sent to Canvas unchanged, because a check that cannot read a shape must not refuse it. Whether Canvas accepts a payload this check passes is live-unverified, which is the point: Canvas is the authority on its own question schema.

All thirteen Item Bank operations share the same schema, authority, approval, one explicit Morrow dispatch, and readback rules as other Canvas operations.

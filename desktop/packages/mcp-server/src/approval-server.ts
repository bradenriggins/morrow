...
function html(
  ...
  const next = (limited
    ...
    : `<p>${batch ? `Morrow will apply all ${plans.length} changes and check each result in Canvas. Searching does not change what you approve.` : addingQuestion ? "Morrow will add this question and check it in Canvas." : "Morrow applies these changes and checks them in Canvas."}</p><p class="keep-open">${keepOpenInstruction(platform)}</p><p class="presence-note">To approve, select the button here in Chrome with Morrow Bridge connected. A request sent to this page by another program cannot approve.</p>`).replaceAll("Canvas", platform);

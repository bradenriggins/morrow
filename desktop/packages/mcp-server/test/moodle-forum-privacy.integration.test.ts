...
const SOURCE_BINDING_ID = "moodle:forum";
const PRINCIPAL_FINGERPRINT = "c".repeat(64);
const TOKEN = "moodle-forum-privacy-token-".repeat(3);
...
function result(command: BridgeCommand, digest: string): JsonObject {
  ...
  if (command.toolName !== "moodle_get_forum_posts") throw new Error(`unexpected source tool ${command.toolName}`);
  const unknown = Number(command.arguments.forum_module_id) === 72;
  return {
    schema: "morrow.moodle-browser-result.v1", ok: true, sent: false, status: 200, truncated: false,
    data: { course_id: "2", forum_module_id: String(command.arguments.forum_module_id), forum_id: "8", posts: [
      { id: "101", discussion: "44", parent: "0", author: { user_id: unknown ? "99" : "7", name: unknown ? "Unknown Student" : "Student Name" }, created: 1700000000, modified: 1700000001, subject: "Student Name started", message: "Student Name posted a reply." },
      { id: "102", discussion: "44", parent: "101", author: { user_id: "3", name: "Course Teacher" }, created: 1700000002, modified: 1700000003, subject: "Course Teacher replied", message: "Course Teacher replied to Student Name." },
    ] }, snapshot_digest: "d".repeat(64),
  };
}
...
describe("Moodle Forum public learner privacy", () => {
  it("fails closed without dispatching learner-authored history when Moodle has no complete historical identity dictionary", async () => {
    ...
    const digest = bridgeCatalogDigestForTests(root); const runtime = await GatewayRuntime.connect(configuration(root, directory, port));

...
export function hasDeclaredCanvasReadback(operation) {
...
const EXACT_READBACKS = Object.freeze({
    ...
    ...Object.fromEntries(["courses", "groups"].flatMap((context) => [
...
function readArguments(read, writeArguments, dynamic = {}, fixedArguments = {}, writeData, argumentsFromWrite = {}) {
    const output = {};
    ...
    for (const parameter of read.parameters || []) {
        if (parameter.location !== "path" || Object.hasOwn(output, parameter.inputName))
            continue;
    ...
    for (const parameter of read.parameters || []) {
        if (parameter.location === "path" || !parameter.required || Object.hasOwn(output, parameter.inputName))
            continue;
...
function collectionTargetArgument(write, read) {
    const writeSegments = write.path.split("/").filter(Boolean);
    const readSegments = read.path.split("/").filter(Boolean);
    if (writeSegments.length !== readSegments.length + 1)
        return undefined;
    if (normalizedPath(`/${writeSegments.slice(0, -1).join("/")}`) !== normalizedPath(`/${readSegments.join("/")}`))
        return undefined;
...
function singleRecordValue(wireName, expected) {
...
export function planBrowserReadback(operations, write, args, writeData) {
    ...
    const orderedTargets = Array.isArray(orderList) && orderList.length > 0
        ...
        : undefined;
    if (override?.orderArgument && !orderedTargets)
        return null;
    return {
        schema: "morrow.browser-readback-plan.v1",
 ...
 */
function envelopeNames(envelope) {
    return envelope.endsWith("s") ? [envelope, envelope.slice(0, -1)] : [envelope];
}
function envelopeRecords(value, envelope) {
    ...
    if (Array.isArray(value) && value.length > 0
        && value.every((page) => page && typeof page === "object" && !Array.isArray(page)
            && Array.isArray(page[envelope]))) {
        return value.flatMap((page) => page[envelope]);
    }
    return null;
}
function isEnvelopedCollection(value, envelope) {
    return envelopeRecords(value, envelope) !== null;
}
function collectionEnvelopeKey(path) {
    ...
    while (segments.length && segments.at(-1).startsWith("{"))
        segments.pop();
    const segment = segments.at(-1);
    return segment && /^[a-z][a-z0-9_]*$/.test(segment) ? segment : null;
}
function targetScope(value, targetPath, envelope) {
...
function targetRecords(value, target, targetField, targetPath, envelope) {
...
export function evaluateBrowserReadback(plan, readResult) {
    ...
    if (plan.strategy === "collection-order") {
        if (readResult.truncated === true)
            return verification("unconfirmed", plan, "collection_readback_incomplete");
        if (!Array.isArray(readResult.data) || !plan.orderedTargets?.length)
            return verification("unconfirmed", plan, "collection_readback_shape_invalid");
    ...
    if (!plan.targetId && records.length !== 1)
        return verification("unconfirmed", plan, "readback_target_unresolved");

...
Space tokens `--space-1` to `--space-7` are 4, 8, 12, 16, 24, 32, and 48 px. Radius tokens are `--radius-control: 10px`, `--radius-card: 14px`, `--radius-panel: 18px`, and `--radius-pill: 999px`; a radius inside a padded container equals the outer radius minus the padding, and never less than `--radius-control`.

A paragraph, a list item, and help text fill the full width of the box they sit in, aligned with the heading and controls beside them. Only a surface's shell (the popup frame, the review card, the settings or setup page width) bounds a line; no rule caps reading text on its own, because a per-text cap wraps supporting text early and makes every section taller. A heading uses `text-wrap: balance`. A paragraph wraps normally, never `pretty` or `balance`, because both shorten a short paragraph's lines to even them out, which is the early wrap this rule forbids. Each surface's root sets `font-synthesis: none` and `-webkit-font-smoothing: antialiased`, so a variable font never synthesizes a weight it does not carry. A count or a clock time uses `font-variant-numeric: tabular-nums`, so digits do not shift width as they change. `::selection` paints `--highlight` behind `--ink` text.

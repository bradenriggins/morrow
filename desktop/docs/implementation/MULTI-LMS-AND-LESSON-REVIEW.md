...
| Blackboard REST | Five reads: own course memberships, exact course, top-level content, direct content children, and exact content. One change: the title, description, or student visibility of one exact `resource/x-bb-document`, with every other protected field frozen and re-checked. | Document bodies, because Original HTML and Ultra BbML are separate native contracts. Also recursive inventory, assessment or bank authoring, browser-cookie authentication, OAuth registration or token renewal, and live-tenant proof. |
